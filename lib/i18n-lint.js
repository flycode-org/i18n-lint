 ...
 * @param {String}      scope Contents of `line` from source file
 * @param {String}       line The line number
 * @param {Boolean} continued Whether the error is continued in multiple lines
 */
var error = function (code, reason, evidence, text, character, scope, line, continued) {
...
var reportErrors = function () {
  ...
  if (Object.keys(lineError).length) {
    if (currentTag.error && I18NLINT.lineNumber === currentTag.error.line) {
      ...
      var continued = Boolean(currentTag.error);
      currentTag.error = error(
        'W001',
        'Hardcoded ' +
          (currentTag.name ? '<' + currentTag.name + '> tag' : 'text node') +
          (continued ? ' (continued)' : ''),
        lineError.evidence,
        lineError.text,
        lineError.character,
...
I18nLint.scan = function (lines, options) {
  ...
  /* Backwards-compatibility from when API only supported a single set of
   * delimiters */
  if (!Array.isArray(options.templateDelimiters[0])) {
    options.templateDelimiters = [options.templateDelimiters];
  }
  ...
  var parser = new htmlparser.Parser(
    {
      ...
      ontext: function (text) {
        ...
        if (
          ...
          if (!Object.keys(lineError).length) {
            ...
            lineError.evidence = utils.appendRegExp(
              lineError.evidence,
      ...
      onattribute: function (name, value) {
        ...
        if (
          ...
          scope = originalLines
            .slice(I18NLINT.lineNumber - offset, I18NLINT.lineNumber)
            .join('\n');
           ...
           * further processing
           */
          lines[I18NLINT.lineNumber - 1] = scope.replace(
            evidence,
            function (text) {
              return [].map
                .call(text, function () {
...
fs.readdirSync(__dirname + '/reporters').forEach(function (file) {
  I18nLint.reporters[file.replace(/\.[a-z]+$/, '')] = require(__dirname +
    '/reporters/' +
    file);
});
